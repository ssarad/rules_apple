...
def tvos_dynamic_framework_test_suite(name):
    ...
    # Test that tvos_dynamic_framework works without explicit infoplists
    infoplist_contents_test(
        name = "{}_no_infoplist_has_default_values_test".format(name),
        target_under_test = "//test/starlark_tests/targets_under_test/tvos:dynamic_fmwk_minimal_no_infoplist",
        expected_values = {
            "CFBundleIdentifier": "com.google.example.framework",
