...
""",
    ...
    # Test that ios_sticker_pack_extension works without explicit infoplists
    analysis_target_tree_artifacts_outputs_test(
        name = "{}_no_infoplist_builds_test".format(name),
        target_under_test = "//test/starlark_tests/targets_under_test/ios:sticker_ext_minimal_no_infoplist",
        expected_outputs = ["sticker_ext_minimal_no_infoplist.appex"],
        tags = [name],
    )
    ...
    infoplist_contents_test(
        ...
        expected_values = {
            "CFBundleIdentifier": "com.google.example.stickerext",
            "CFBundleName": "sticker_ext_minimal_no_infoplist",
            "CFBundlePackageType": "XPC!",
        },
        tags = [name],
    )
